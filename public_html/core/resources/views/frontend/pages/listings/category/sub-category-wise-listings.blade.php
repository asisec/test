...
@section('site-title')
    @if($subcategory !='')
        {{ deepl_translate(__($subcategory->name)) }}
    @endif
@endsection
@section('page-title')
    @if($subcategory !='')
    ...
    <div class="catagory-wise-listing section-padding2">
        <div class="container-1440">
            <x-breadcrumb.user-profile-breadcrumb
                :title="''"
                :innerTitle="deepl_translate(__($subcategory->category?->name))"
                :subInnerTitle="deepl_translate(__($subcategory->name))"
                :chidInnerTitle="''"
            ...
            <h3 class="catagory-wise-title">{{ deepl_translate(__($subcategory->name)) }} {{ deepl_translate(__('Available Subcategories')) }}</h3>
                              ...
                              <div class="singleCategories categories1 wow fadeInUp" data-wow-delay="0.1s">
                                            ...
                                            <div class="categorie-text">
                                                <h4 class="text-center">
                                                    ...
                                                    {{ deepl_translate($child_cat->name) }}
                                                    </a>
                                                </h4>
                                                <p> {{ deepl_translate(__('Listing :total_listings', ['total_listings' => $child_cat->total_listings ?? 0])) }}</p>
                                            </div>
                                    </div>
                            @endforeach
                        @else
                    ...
                    <section class="featureListing mb-5 mt-5">
                        <div class="container-1440">
                            <div class="titleWithBtn d-flex justify-content-between align-items-center mb-40">
                                <h3 class="catagory-wise-title">{{ deepl_translate(__($subcategory->name)) }} {{ deepl_translate(__('Available Listings')) }}</h3>
