...
@section('site_title', deepl_translate(__('User Login')))
@section('style')
    <style>
        .table {
            ...
            <div class="row">
                <div class="col-xl-5 col-lg-5 p-0 order-lg-1 order-1 loginLeft-img">
                    <div class="loginLeft-img">
                        <div class="login-cap">
                            <h3 class="tittle">{{ get_static_option('login_page_title') ?? deepl_translate(__('Buy & sell anything')) }}</h3>
                            <p class="pera">{{ get_static_option('register_page_description') ?? deepl_translate(__('Buy or Sell your any items.')) }}</p>
                        </div>
                        <div class="login-img">
                ...
                <div class="col-xl-7 col-lg-7 order-lg-1 order-0 login-Wrapper">
                    ...
                    <div class="row">
                        <form method="post" action="{{ route('user.login') }}">
                            @csrf
                            <div class="col-md-12">
                                <label class="infoTitle">{{ deepl_translate(__('Email Or User Name')) }}</label>
                                <div class="input-form">
                                    <input type="text" name="username" id="username" placeholder="{{ deepl_translate(__('Email Or User Name')) }}">
                        ...
                        <div class="col-sm-12">
                            <div class="passRemember mt-20">
                                <label class="checkWrap2">{{ deepl_translate(__('Remember Me')) }}
                                    <input class="effectBorder" name="remember"  type="checkbox" id="check15">
                                    <span class="checkmark"></span>
                                </label>
                                <!-- forgetPassword -->
                                <div class="forgetPassword mb-25">
                                    <a href="{{ route('user.forgot.password') }}" class="forgetPass">{{ deepl_translate(__('Forgot Password?')) }}</a>
                                </div>
                            </div>
                        </div>
                        <div class="col-sm-12">
                            <div class="btn-wrapper text-center">
                                <button id="signin_form" class="cmn-btn4 w-100">{{ get_static_option('login_page_button_title') ?? deepl_translate(__('Login')) }}</button>

                                @if(preg_match('/(bytesed)/',url('/')))
                                    <div class="adminlogin-info table-responsive margin-top-30">
                                        <table class="table">
                                            <th>{{deepl_translate(__('Username'))}}</th>
                                            <th>{{deepl_translate(__('Password'))}}</th>
                                            <th>{{deepl_translate(__('Action'))}}</th>
                                            <tbody>
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="btn-wrapper form-icon">
                                                         <button type="button" class="red-btn user_login_demo_button autoLogin" id="userLogin">{{deepl_translate(__('Login'))}}</button>
                                                    </div>
                                                </td>
                                            </tr>
                                            </tbody>
                                ...
                                @if(moduleExists('SMSGateway') && get_static_option('otp_login_status') || !empty(get_static_option('register_page_social_login_show_hide')) && get_static_option('enable_google_login') || get_static_option('enable_facebook_login'))
                                       <p class="font-weight-bold text-center mt-2 mb-2">{{deepl_translate(__('or'))}}</p>
                                @endif
                                @if(moduleExists('SMSGateway') && get_static_option('otp_login_status'))
                                    <a href="{{route('user.login.otp')}}" class="cmn-btn-outline4 w-100 mb-20">{{deepl_translate(__('Login In with OTP'))}}</a>
                                @endif
                                <!--social login -->
                                ...
                                <p class="sinUp"><span>{{ deepl_translate(__('Don’t have an account?')) }}</span><a href="{{ route('user.register') }}" class="singApp">{{ deepl_translate(__('Sign Up')) }}</a></p>
                            </div>
                        </div>
        ...
        (function ($) {
            ...
            $(document).ready(function () {
                ...
                $(document).on('click','#signin_form',function (e){
                    ...
                    erContainer.html('');
                    el.text('{{deepl_translate(__('Please Wait..'))}}');
                    $.ajax({
                        ...
                        error:function(data){
                            ...
                            el.text('{{deepl_translate(__('Login'))}}');
                        },
                        success:function (data){
                            $('.alert.alert-danger').remove();
                            if (data.status == 'user-login'){
                                el.text('{{deepl_translate(__('Redirecting'))}}..');
