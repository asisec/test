...
<div class="seller-phone text-center">
    ...
    @if($listing->phone_shows_only_premium === 1)
        @if($userHasMembership === true)
            ...
            <a href="javascript:void(0)" class="show-number callPhoneNumberBtn" id="userPhoneNumberBtnForResponsive">{{ get_static_option('listing_show_phone_number_title') ?? __('Show Number') }}</a>
        @else
            @include('frontend.pages.listings.listing-premium-required-modal-responsive')
            <a href="javascript:void(0)" class="show-number" data-bs-toggle="modal" data-bs-target="#premiumRequiredModalResponsive">{{ get_static_option('listing_show_phone_number_title') ?? __('Show Number') }}</a>
        @endif
