...
class UserListingManageController extends Controller
{
    ...
    public function listingDetails($id, Request $request){
        ...
        if ($request->isMethod('post')) {
            $request->validate([
                'phone_shows_only_premium' => 'required|numeric',
            ], [
                'phone_shows_only_premium.required' => __('The price field is required.'),
                'phone_shows_only_premium.numeric' => __('The price must be a numeric value.')
            ]);

            $listing_update =  Listing::findOrFail($id);
            $listing_update->phone_shows_only_premium = +!!$request->phone_shows_only_premium;
