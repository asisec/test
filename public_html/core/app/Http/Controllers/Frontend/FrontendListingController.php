    ...
    public function frontendListingDetails($slug)
    {
        ...
        if(moduleExists('Membership')){
            if(membershipModuleExistsAndEnable('Membership')){
                ...
                if ($user) {
                    $user_membership_check = UserMembership::where('user_id', $user->id)->first();
                    $user_membership = UserMembership::where('user_id', $user->id)->first();
                    if (
                        !is_null($user_membership) &&
                        $user_membership_check &&
                        $user_membership_check->status === 1 &&
                        $user_membership_check->payment_status == 'complete' && 
                        $user_membership_check->price > 0
                    ) {
